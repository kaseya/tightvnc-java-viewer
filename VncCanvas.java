...
implements KeyListener, MouseListener, MouseMotionListener {
	...
	public void destroy() {
		...
		System.out.println("VncCanvas destroyed.");
	}
	//
	...
	public void paintScaledFrameBuffer(Graphics g) {
		g.drawImage(memImage, 0, 0, (int)((scaledWidth * horizontalScaleFactor) + 0.5), (int)((scaledHeight * verticalScaleFactor) + 0.5), null);
	}
	...
	void updateFramebufferSize() {
        ...
        // ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Kaseya International Limited modification
        // Data buffers are nulled before re-use to trigger garbage collection.
        // ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if (bytesPixel == 1) {
	...
	throws IOException {
		...
		if (bytesPixel == 1) {
			...
			for (int dy = y; dy < y + h; dy++) {
                ...
                //System.out.println("Offset" + offset + "pixel24 size" + pixels24.length);
				for (i = 0; i < w; i++) {
                  //  System.out.println("Buffer index" + i + " " + buf[i *4 + 2] + " " + buf[i *4 + 1] + buf[i *4]);
						pixels24[offset + i] =
						(buf[i * 4 + 2] & 0xFF) << 16 |
						(buf[i * 4 + 1] & 0xFF) << 8 |
						(buf[i * 4] & 0xFF);
	...
	void handleTightRect(int x, int y, int w, int h) throws Exception {
		...
		if (comp_ctl == rfb.TightJpeg) {
			...
			jpegRect = new Rectangle((int)(x * horizontalScaleFactor), (int)(y * verticalScaleFactor),
									 (int)(w * horizontalScaleFactor ) + 2, (int)(h * verticalScaleFactor) + 2);
	...
	void scheduleRepaint(int x, int y, int w, int h) {
		...
		} else if ( horizontalScaleFactor != 1.0 || verticalScaleFactor != 1.0)	{
			repaint(viewer.deferScreenUpdates,(int)(x * horizontalScaleFactor),(int)(y * verticalScaleFactor),
					                          (int)(w * horizontalScaleFactor) + 2, (int)(h * verticalScaleFactor) + 2);
		} else {
	...
	public void processLocalMouseEvent(MouseEvent evt, boolean moved) {
		if (viewer.rfb != null && rfb.inNormalProtocol) {
			...
			if ( horizontalScaleFactor != 1.0 || verticalScaleFactor != 1.0 ) {
				int remoteX = (int)( (evt.getX()/horizontalScaleFactor) + 0.5);
				int remoteY = (int)( (evt.getY()/verticalScaleFactor) + 0.5);
				evt.translatePoint(remoteX - evt.getX(), remoteY - evt.getY());
			} else if ( rfb.framebufferWidth != scaledWidth ) {
	...
	synchronized void fitAppletToWindow(boolean enable)	{
		if( enable ) {
			fitToWindow=true;
			Dimension size = viewer.getSize( );
			adjustSize( size.width, size.height - (viewer.buttonPanel == null ? 0 : viewer.buttonPanel.getHeight()) );
		} else {
			fitToWindow = false;
	...
	synchronized public void adjustSize(int width, int height) {
	  if( fitToWindow ) {
                scaledWidth = width;
                scaledHeight = height;
                horizontalScaleFactor = (double)width/(double)rfb.framebufferWidth;
                verticalScaleFactor = (double)height/(double)rfb.framebufferHeight;
                System.out.println("VncCanvas::adjustSize(on): width=" + scaledWidth + " height=" + scaledHeight + " horizontalFactor=" + horizontalScaleFactor + " verticalFactor=" + verticalScaleFactor);
                setSize(scaledWidth, scaledHeight);
                //updateFramebufferSize();
